...
export async function activateBenchmarkSchedule(id: number, _rawCookie?: string) {
  ...
  const taskUid = `task-${schedule.id}-${Date.now()}`;
  return db.activateBenchmarkSchedule(schedule.id, taskUid);
}
...
export async function executeBenchmarkSchedule(schedule: BenchmarkSchedule, _options: { notify?: boolean } = {}) {
